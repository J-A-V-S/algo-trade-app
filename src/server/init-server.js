...
import AngelBroking from './brokers/angelBroking/AngelBroking';
...
const parseAngelBroking = (req) => {
  if (!req.query.broker) return false;
  if (req.query.broker.startsWith('angelbroking?auth_token=')) {
    req.query.request_token = req.query.broker.replace('angelbroking?auth_token=', '');
    return 'angelbroking';
  }
  return false;
};
...
const getBrokerFromRequest = (req) => {
  logger.info("Line 96");
  logger.info(req.query.broker);
  let broker = parseAngelBroking(req) ||_.get(req, 'query.broker');
  if (_.isEmpty(broker)) {
...
app.get('/apis/broker/logincheck', isAuthenticated, (req, res) => {
  ...
  switch (broker) {
    ...
    case 'angelbroking':
      res.status(200).send({
        isLoggedIn: AngelBroking.isLoggedIn()
      });
      break;
